...
public class GameM2 implements ActionListener {
    ...
    public GameM2(){
		...
		menu2.setBackground(new Color(160,32,240));
		...
		easy.setFont(new Font("Consolas", Font.BOLD, 16));
		mini.add(hard, BorderLayout.NORTH);
		hard.setFont(new Font("Consolas", Font.BOLD, 16));
		mini.add(inst, BorderLayout.SOUTH);
		inst.setFont(new Font("Consolas", Font.BOLD, 16));
		menu2.add(start);
		start.setFont(new Font("Consolas", Font.BOLD, 16));
		menu2.add(over);
		over.setFont(new Font("Consolas", Font.BOLD, 16));
	...
	public void setUpGame(int x,Boolean mode){
		...
		for(int i=0;i<(x*2);i++){
			btn[i] = new JButton("");
			btn[i].setForeground(new Color(0, 255, 0));
		...
		String[] c = {"rhombus","circle","rectangle","heart","diamond","clover","spade","triangle","polygon","square"};//easier version
		if(mode) a=c;//if mode is true, make the game easy and use c
	...
	public void switchSpot(int i){//this will switch the current spot to either blank or mode it should have
	...
	public void winner(){
			...
			end_screen.add(new TextField("No. of tries: " + score), BorderLayout.SOUTH);
			end_screen.add(goBack);
			goBack.setEnabled(true);
			goBack.setFont(new Font("Consolas", Font.BOLD, 16));
...
public void actionPerformed(ActionEvent click){
		...
		if(source==inst){//this just sets the instruction screen
			...
			JScrollPane taScroll = new JScrollPane(instructM);
			one.add(taScroll);
			two.add(goBack);
			goBack.addActionListener(this);
			goBack.setEnabled(true);
	...
	public static void main(String[] args) {
		new GameM2();// Calling the class construtor.
	}
